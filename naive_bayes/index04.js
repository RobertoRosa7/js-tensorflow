...
function executar() {
  let execucao = [];
  let selEntrada = $('#sel_entrada').val().toString().trim();
  if (selEntrada.length > 0) {
    $('#resultado').html('...carregando.');
    execucao = arrayStringToNumber([selEntrada]);
    ...
    model.fit(x, y, { epochs: 500 }).then(() => {
      ...
      $('#resultado').html(`- CLASSIFICAÇÃO: <strong>${output}</strong>`)
    });
  }
}
...
function arrayStringToNumber(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let element = arr[i];
    if (element == 'bom' || element == 'positivo') {
      element = 1;
    } else {
      element = 0;
    }
